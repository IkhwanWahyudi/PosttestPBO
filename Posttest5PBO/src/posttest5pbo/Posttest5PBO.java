...
public class Posttest5PBO {
    ...
    static ArrayList<Riwayat> riwayatTransaksi = new ArrayList<>();
    static String userAktif;
    
    public static void main(String[] args) throws IOException {
        User admin = new User("admin", "admin", "null", "null");
        User user = new User("ikhwan", "123", "Jl. Cendana", "ikhwan@gmail.com");
        dataUser.add(admin);
        dataUser.add(user);
        
        Digital addDgtl = new Digital("High Tide BLACK", "Nixon", "D1G0", 
                "Black", 250000, 200, "LED Light, Stopwatch, Temperature");
        dgtl.add(addDgtl);
        
        Analog addAnlg = new Analog("High Tide BLACK", "Nixon", "4N40", 
                "Black", 250000, 200, "Date");
        anlg.add(addAnlg);
    ...
    public static void menuAdmin() throws IOException{
        ...
        while(!"E".equals(pilih1)) {
            for (int i = 0; i < dgtl.size(); i++){
                if(dgtl.get(i).getStok() == 0){
                    dgtl.remove(i);
                }
            }
            for (int i = 0; i < anlg.size(); i++){
                if(anlg.get(i).getStok() == 0){
                    anlg.remove(i);
                }
            }
            System.out.println("""
                ...
                case "1", "Digital" -> {
                    if(dgtl.size() < 1){
                        ...
                        System.out.println("""
                                            ...
                                            O===============================O""");
                        for (int i = 0; i < dgtl.size(); i++){
                            System.out.println("  Watch No. " + (i+1));
                            if(userAktif.equals("admin")){
                                dgtl.get(i).tampil(true);
                            } else {
                                dgtl.get(i).tampil(false);
                            }
                            System.out.println("---------------------------------");
                        }
                        System.out.println("=================================");
                ...
                case "2", "Analog" -> {
                    if(anlg.size() < 1){
                        ...
                        for (int i = 0; i < anlg.size(); i++){
                            System.out.println("  Watch No. " + (i+1));
                            if(userAktif.equals("admin")){
                                anlg.get(i).tampil(true);
                            } else {
                                anlg.get(i).tampil(false);
                            }
                            System.out.println("---------------------------------");
                        }
                        System.out.println("=================================");
    ...
    public static void transaksi(){
        ...
        for(int i = 0; i < riwayatTransaksi.size(); i++){
            riwayatTransaksi.get(i).tampil();
            System.out.println("---------------------------------");
            total = riwayatTransaksi.get(i).getHargaBarang() * riwayatTransaksi.get(i).getJumlahBarang();
            overal += total;
        }
        System.out.println("  Overall Profit = Rp." + overal);
        System.out.println("---------------------------------");
    }
    ...
    public static void ubah() throws IOException{
        String pilihtipe = "a", nama, merk, warna;
        int stok, harga;
                    ...
                    if(dgtl.size() < 1){
                        ...
                        for (Digital jamDigital : dgtl) {
                            if (jamDigital.getKode().equals(no)){
                                ...
                                jamDigital.SetEdit("Name", nama, 
                                        merk, warna, harga, stok);
                                jamDigital.SetEdit("Merk", nama, 
                                        merk, warna, harga, stok);
                                jamDigital.SetEdit("Warna", nama, 
                                        merk, warna, harga, stok);
                                jamDigital.SetEdit("Harga", nama, 
                                        merk, warna, harga, stok);
                                jamDigital.SetEdit("Stok", nama, 
                                        merk, warna, harga, stok);
                    ...
                    if(anlg.size() < 1){
                        ...
                        for (Analog jamAnalog : anlg) {
                            if (jamAnalog.getKode().equals(no)){
                                ...
                                jamAnalog.SetEdit("Name", nama, 
                                        merk, warna, harga, stok);
                                jamAnalog.SetEdit("Merk", nama, 
                                        merk, warna, harga, stok);
                                jamAnalog.SetEdit("Warna", nama, 
                                        merk, warna, harga, stok);
                                jamAnalog.SetEdit("Harga", nama, 
                                        merk, warna, harga, stok);
                                jamAnalog.SetEdit("Stok", nama, 
                                        merk, warna, harga, stok);
                ...
                case "1", "Digital" -> {
                    if(dgtl.size() < 1){
                        ...
                        if (beli <= dgtl.size() || beli > 0){
                            ...
                            String ulang = "a";
                            while(!"E".equals(ulang)){
                                System.out.print("  Purchase amount    : ");
                                int jumlah = Integer.parseInt(input.readLine());

                                if (jumlah > dgtl.get(beli-1).getStok() || jumlah <= 0){
                                    System.out.println("  -> Invalid Number <-  ");
                                    ulang = "a";

                                } else {
                                    Transaksi buy = new Transaksi(namaCust, barang, 
                                            merk, kode, warna, jumlah, harga, fitur);
        ...
        if (cekKeranjang != 0){
            ...
            for (int i = 0; i < dataTransaksi.size(); i++){
                if (dataTransaksi.get(i).getNamaCustomer().equals(userAktif)){
                    ...
                    System.out.println(totalSatuan);
                    total += totalSatuan;
                    System.out.println("=================================================================");
                }
            }
            ...
            for (int i = 0; i < dataTransaksi.size(); i++){
                if (dataTransaksi.get(i).getNamaCustomer().equals(userAktif)){
                    for (Analog jamAnalog : anlg) {
                        if (jamAnalog.getKode().equals(dataTransaksi.get(i).getKodeBarang())){
                            
                            String nama = dataTransaksi.get(i).getNamaCustomer(),
                                    merk = dataTransaksi.get(i).getMerkBarang(),
                                    warna = dataTransaksi.get(i).getWarnaBarang(),
                                    barang = dataTransaksi.get(i).getNamaBarang();
                            int harga = dataTransaksi.get(i).getHargaBarang(),
                                    stok = jamAnalog.getStok() - dataTransaksi.get(i).getJumlahBarang();
                            jamAnalog.SetEdit("Stok", nama, merk, 
                                    warna, harga, stok);
                            
                            Riwayat addRiwayat = new Riwayat(nama, barang, 
                                            merk, dataTransaksi.get(i).getKodeBarang(), 
                                    warna, dataTransaksi.get(i).getJumlahBarang(), harga,
                                    dataTransaksi.get(i).getFitur());
                            riwayatTransaksi.add(addRiwayat);
            ...
            for (int i = 0; i < dataTransaksi.size(); i++){
                if (dataTransaksi.get(i).getNamaCustomer().equals(userAktif)){
                    ...
                    for (Digital jamDigital : dgtl) {
                        if (jamDigital.getKode().equals(dataTransaksi.get(i).getKodeBarang())){
                            ...
                            String nama = dataTransaksi.get(i).getNamaCustomer(),
                                    ...
                                    barang = dataTransaksi.get(i).getNamaBarang();
                            int harga = dataTransaksi.get(i).getHargaBarang(),
                                    stok = jamDigital.getStok() - dataTransaksi.get(i).getJumlahBarang();
                            jamDigital.SetEdit("Stok", nama, merk, 
                                    warna, harga, stok);
